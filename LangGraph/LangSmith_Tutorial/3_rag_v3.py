...
load_dotenv()
os.environ['LANGCHAIN_PROJECT'] = '3_rag_v3.py'

PDF_PATH = "data/islr.pdf"

# ----------------- helpers (traced individually) -----------------
@traceable(name="load_pdf", tags=["pdf", "loader"], metadata={"step": "load PDF"})
def load_pdf(path: str):
    loader = PyPDFLoader(path)
    return loader.load()  # returns list[Document]

@traceable(name="split_documents", tags=["pdf", "splitter"], metadata={"step": "split PDF into chunks"})
...
@traceable(name="build_vectorstore", tags=["vectorstore", "embedding"], metadata={"embedding_model": "all-MiniLM-L6-v2"})
def build_vectorstore(splits):
    emb = SentenceTransformerEmbeddings(model_name="all-MiniLM-L6-v2")
    return FAISS.from_documents(splits, emb)

# ----------------- parent setup function (traced) -----------------
@traceable(name="setup_pipeline", tags=["setup", "pipeline"], metadata={"chunk_size": 1000, "chunk_overlap": 150})
...
@traceable(name="pdf_rag_full_run", tags=["full_run", "rag"], metadata={"llm_model": "llama3.2", "temperature": 0})
def setup_pipeline_and_query(pdf_path: str, question: str):
    ...
    lc_config = {"run_name": "3_rag_v3", "tags": ["query", "interactive"]}
